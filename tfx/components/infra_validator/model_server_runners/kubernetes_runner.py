...
class KubernetesRunner(base_runner.BaseModelServerRunner):
  ...
  def __init__(
    ...
    self._model_base_path = os.path.dirname(  # base
        os.path.dirname(  # base/model
            path_utils.serving_model_path(model.uri)))  # base/model/version
  ...
  def __repr__(self):
    return 'KubernetesRunner(image: {image})'.format(
  ...
  def Start(self) -> None:
    ...
    # We're creating Deployment rather than a Pod, as Deployment automatically
    # recovers the pod if it fails or preempted, providing a minimal amount of
    # fault tolerance.
    ...
    while time_utils.utc_timestamp() < deadline:
      ...
      if pod_phase == _POD_PHASE_RUNNING and pod.status.pod_ip:
        self._endpoint = '{}:{}'.format(pod.status.pod_ip,
                                        self._binary_kind.container_port)
        return
  ...
  def _BuildPodManifest(self) -> k8s_client.V1Pod:
    if isinstance(self._binary_kind, binary_kinds.TensorFlowServing):
      env_vars = self._binary_kind.MakeEnvVars(
