...
# TODO(zhuo): unconditionally import once TFX 0.22 is released.
...
class Executor(base_executor.BaseExecutor):
  ...
  def _GenerateStats(
    ...
    # TODO(b/DO NOT SUBMIT: file a bug): Remove once TFDV starts accepting
    # RecordBatches.
    if input_from_tfxio:
  ...
  def Transform(self, inputs: Mapping[Text, Any], outputs: Mapping[Text, Any],
    ...
    if use_tfxio:
      ...
      for d in all_datasets:
        self._CreateTFXIO(d, input_dataset_metadata.schema)
  ...
  def _RunBeamImpl(self,
                   ...
                   materialization_format: Text) -> _Status:
    ...
    Args:
      ...
      enable_materialization: A bool indicating whether to materialize
        transformed data.
    ...
    with self._CreatePipeline(transform_output_path) as pipeline:
      with tft_beam.Context(**beam_context_kwargs):
        ...
        # materialization.
        if not enable_materialization and not compute_statistics:
        ...
        if compute_statistics or enable_materialization:
          ...
          for dataset in transform_data_list:
            ...
            if enable_materialization or not stats_use_tfdv:
  ...
  def _CreateTFXIO(self, dataset: _Dataset, schema: schema_pb2.Schema) -> None:
    """Creates a TFXIO instance for `dataset`."""
    if self._ShouldDecodeAsRawExample(dataset.data_format):
      dataset.tfxio = raw_tf_record.RawTfRecordTFXIO(
          dataset.file_pattern, RAW_EXAMPLE_KEY)
    else:
      dataset.tfxio = tf_example_record.TFExampleRecord(
          dataset.file_pattern,
          # TODO(b/114938612): Eventually remove this override.
          validate=False,
          schema=schema)
  ...
  def _AssertSameTFXIOSchema(self, datasets: Sequence[_Dataset]) -> None:
    ...
    for dataset in datasets[1:]:
      assert (
          datasets[0].tfxio.ArrowSchema().equals(dataset.tfxio.ArrowSchema()))
